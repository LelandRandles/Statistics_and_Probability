...
Exercise 2 Answer:  There was a drop in the number of girls baptized between 1640-1650, then a steady, low number of girls baptized from 1650-1660, and then from 1660 on there was a gradual rise in the number of girls baptized except for a few spiked declines (i.e., around 1702 or so).
...
Exercise 3 Answer:
```{r plot-proportion-boys-vs-year, eval=TRUE}
...
```{r oyo1, eval=TRUE}
# Years included in data set (1940 to 2002)
present$year
# Dimenstions of data set (63 rows, 3 columns)
dim(present)
# Names of columns (year, boys and girls)
names(present)
```

Number 2 Answer: 

The counts are similar.  Same number of variables, 82 observations for the Arbuthnot data frame and 63 for the present data frame.  As for scale, they are not similar, the number of boys and girls included in the sample per year is dramatically larger for the 'present' data frame compared to the Arbuthnot data frame.
   
Number 3 Answer:

```{r oyo3, eval=TRUE}
# Yes, the present data frame does show boys being born in greater
# proportion than girls, and the ratio is even fairly similar
plot(present$year, present$boys / (present$boys + present$girls), type = "l")
``` 

Number 4 Answer:
```{r oyo4, eval=TRUE}
present[which(present$boys + present$girls == max(present$boys + present$girls)),"year"]
```
