...
*Exercise 1 Answers:*
A streak of one means a hit (H) followed by a miss (M).  A streak of zero is a single miss (M) which came after a miss (M).  
...
*Exercise 2 Answers:*
The distribution of Kobe's streak lengths is right-skewed. His typical streak length is 0 or 1, though perhaps we should not call zeroes a streak at all, so I'll say 1.  His longest streak of baskets was 4.  
...
*Answers*  
```{r oyo, EVAL = TRUE}
# First I ran calc_streak per the instructions
...
The distribution of streak lengths is right-skewed, with a typical streak length of 0 or 1, though like before, I am not sure we should call zeroes a streak, so I'll say the typical streak is 1.  The longest streak of baskets was 5.  

*Number 2 Answer:*  
I would expect the streak distribution to be different, but somewhat similar, not totally different.  The reason I feel that way is because there will be variability from simulation to simulation, but with a sample size of 133 shots, the variability is not likely to be huge.  

*Number 3 Answer:*  
The distribution for the simulation is quite similar to Kobe's distribution of streak lengths.  As such, the evidence refutes the hot hand model.  We would expect the hot hand model to be true if we DID NOT see a similar distribution between the kobe streak distribution and the independent simulated streak distibution.  
