...
A scatterplot is an effective way to visualize potential relationships between two variables.  
...
You can see an upward right trend, but it doesn't appear to be a very strong relationship. The points are scattered fairly widely. There are a few outliers as well, like the one on the upper left.  
...
The lowest I could come up with was 137227.3. I went across the hall to my neighbor's condo to see if they could try it, so I could compare, but unfortunately they were not home.  ;-)  
...
It looks like there is a strong positive correlation between homeruns and runs scored.
...
y = 727.6861 = -2789.2429 + (0.6305 * 5578)

There is no exact observation for at_bats = 5,578. The closest observation is 5,579. The residual for that observation is shown below:
...
No apparent pattern. Pattern is weak enough that it may mean the slope of the linear model is not statistically significant.  
...
-   Choose another traditional variable from `mlb11` that you think might be a 
    good predictor of `runs`. Produce a scatterplot of the two variables and fit     a linear model. At a glance, does there seem to be a linear relationship?  
...
```{r oyo1, eval=TRUE}
# Scatterplot of on-base plus slugging vs. runs
plot(x = mlb11$new_obs, y = mlb11$runs)
# Linear model
m1 <- lm(runs ~ new_obs, data = mlb11)
```  
  
Yes, there appears to be a pretty strong linear relationship.  
...
This linear relationship is much stronger than the one between `at_bats` and `runs`. The R^2 is much higher and the p-value is extremely low.  
  

*Number 3 Answer:*  
  
The best was `bat_avg`:

```{r oyo3, eval=TRUE}
# Scatterplot for batting average vs runs
...
Yes, in general, all three newer variables are better predictors of runs. The best is `new_obs`, which is what I selected for my answers to 1 and 2 (see those for graphs and numerical evidence). Yes, it does make sense that these stats would be better predictors than the traditional stats, based on my knowledge of baseball.  
