...
They are sample statistics derived from the data sample.  
...
To generalize the report's findings to the global human population, we must assume that the sample was representative of the global human population. The report says that a "national probability sample" was taken for each country, which means that 
every individual in each country had an equal chance of being selected to be surveyed. The sampling method seems reasonable.
...
Each row of Table 6 corresponds to the sample of survey responses from a given country. Each row of `atheism` corresponds to one survey response from an individual.  
...
```{r ex4, eval = TRUE} 
# Proportion of atheist responses
nrow(subset(us12, response == "atheist")) / nrow(us12)
# The table on page 16 says 5% of United States survey responders 
# described themselves as "A convinced atheist". Our calculation is not
#  exactly the same, but it is pretty close. The difference is that 
# the Table on page 16 uses rounding (see footnote at bottom of table).
```  
...
The observations must be independent, and the success-failure condition must be met. A national probability poll was used, which satisfies the independence condition, and there are much greater than 10 successes and 10 failures in the sample. I am confident all conditions are met.  
...
The margin of error is the max value of the confidence interval minus the min value divided by 2, or (0.0634 - 0.0364) / 2 = 0.027.  
...
```{r ex7, eval=TRUE}
# Create two data frames for Argentina and Poland
a12 <- subset(atheism, nationality == "Argentina" & year == "2012")
p12 <- subset(atheism, nationality == "Poland" & year == "2012")

# Argentina inference function
inference(a12$response, est = "proportion", type = "ci", method = "theoretical",   success = "atheist")
# Argentina margin of error
(0.0866 - 0.0547) / 2
# Poland inference function
inference(p12$response, est = "proportion", type = "ci", method = "theoretical",   success = "atheist")
# Poland margin of error
(0.0681 - 0.031) / 2
# Based on independence of observations and the number of successes and failures
# being over 10 for both Argentina and Poland, the conditions for inference are met
```  
...
The maximum margin of error occurs when the population proportion is 0.5, and becomes lower at a negative exponential rate as it moves away from 0.5 towards 0 or 1.  
...
The mean is shown above. The distribution is fairly close to normal and the spread is 0.05961.  
...
hist(p_hats, main = "p = 0.1, n = 1040", xlim = c(0, 0.18))
# Histogram with n=400 and p=0.1
p <- 0.1
n <- 400
p_hats <- rep(0, 5000)
for(i in 1:5000){
...
# Histogram with n=1040 and p=0.02
p <- 0.02
n <- 1040
p_hats <- rep(0, 5000)
for(i in 1:5000){
...
# Histogram with n=400 and p=0.02
p <- 0.02
n <- 400
p_hats <- rep(0, 5000)
for(i in 1:5000){
...
The p = 0.1, n = 400 histogram has the same mean and shape as p = 0.1, n = 1040, but it is wider. Both p = 0.02 charts have their center at 0.02, but follow the same pattern - when n = 400, the distribution is wider than when n = 1040. n affects the spread of the distribution. The lower n is, the greater the spread and wider the distribution. p affects the location of the center of the distribution.  
...
It does not seem sensible for the report to make generalized statements about margin of error for all countries, or to proceed with inference.  
...
```{r oyo1a, eval=TRUE, tidy=FALSE}
# Create Spain data frames for 2005 and 2012
sp05 <- subset(atheism, nationality == "Spain" & year == "2005")
sp12 <- subset(atheism, nationality == "Spain" & year == "2012")
# Run inference function for each data frame
...
# The confidence intervals for both populations heavily overlap, so there
# is not convincing evidence that Spain has a seen a change in its
# atheism index between 2005 and 2012.
```  
...
```{r oyo1b, eval=TRUE, tidy=FALSE}
# Create US data frames for 2005 (us12 was already created previously)
us05 <- subset(atheism, nationality == "United States" & year == "2005")
# Run inference function for each data frame
inference(us05$response, est = "proportion", type = "ci", method = "theoretical",   success = "atheist")
inference(us12$response, est = "proportion", type = "ci", method = "theoretical",   success = "atheist")
# The confidence intervals for both populations do not overlap, so there
# is convincing evidence that the United States has a seen a change in its
# atheism index between 2005 and 2012.
```  
...
$\approx 2$. About 5% of the # of countries, which is 39.
  
  
*Number 3 Answer:*  
  
```{r oyo3, eval=TRUE, tidy=FALSE}
# n should be greater than or equal to the results of the 
# following formula rounded up to the nearest whole number
