...
See histograms below.  The two distributions are both fairly normal.  The men's distribution has a higher mean and a wider ranges of values, including more outliers, than the women's distribution.  

```{r hgs, eval=TRUE}
hist(mdims$hgt, breaks = 20, main = "Distribution of Men's Heights", xlab = "Height in Centimeters")
hist(fdims$hgt, breaks = 20, main = "Distribution of Women's Heights", xlab = "Height in Centimeters")
```
...
Yes, the data does appear to follow a nearly normal distribution.  
...
See plot below. All of the points do not fall on the line. The plot points follow the line no more accurately than the plot for the real data, and we see errant points at the tails just like we saw when we plotted the real data.  If anything, I would say the plot for the real data conforms to the line more closely than the plot for the simulated data.  
...
Yes, the normal probability plot for female heights DOES look similar the simulated plots.  I do feel that the plots provide evidence that female heights are nearly normal.  
...
See R code and plots below.  Female weights do seem to fit a normal distribution, but not as closely as female heights did.  

```{r fem_wgt, eval=TRUE}
# Start by creating QQ plots as was done for female heights  
qqnorm(fdims$wgt)
qqline(fdims$wgt) 
# Calculate mean and sd for female weights  
fwgtmean <- mean(fdims$wgt)
fwgtsd   <- sd(fdims$wgt)
# Create normal simulated data using mean and sd of female weights data  
wsim_norm <- rnorm(n = length(fdims$wgt), mean = fwgtmean, sd = fwgtsd)
# Plot the simulated data
qqnorm(wsim_norm)
qqline(wsim_norm)
# Create multiple simulated plots using Dr. Bryer's mind-bending qqnormsim function
qqnormsim(fdims$wgt)
```
...
Probability question 1:  "What is the probability that a randomly chosen young adult female is shorter than 152 cm?"  
...
Probability question 2:  "What is the probability that a randomly chosen young adult female weighs more than 68 kilograms?"  
...
*Number 2 Answer:*  
Because the data is discrete (age) or has been rounded (elbow diameter).  
...
qqnorm(fdims$kne.di)
# Looks like it is skewed to the right.  Histogram confirms:
hist(fdims$kne.di, breaks = 20, main = "Distribution of Female Knees Diameters", xlab = "Diameter in Centimeters")
```  
