...
*Exercise 1 Answer:*
There are 20,000 cases in this data set and 9 variables.  The variables are listed below along with their type.  
genhlth - categorical, ordinal  
exerany - categorical, non-ordinal  
hlthplan - categorical, non-ordinal  
smoke100 - categorical, non-ordinal  
height - numerical, discrete (because rounded to integer; conceptually continuous)  
weight - numerical, discrete (because rounded to integer; conceptually continuous)  
wtdesire - numerical, discrete (because rounded to integer; conceptually continuous)  
age - numerical, discrete (because rounded to integer; conceptually continuous)  
gender - categorical, non-ordinal  
...
table(cdc$gender)
# What proportion of the sample reports being in excellent health? (0.23285)
...
*Exercise 3 Answer:*
It tells us that a larger proportion of males have smoked 100 or more cigarettes in their lifetime than females.
...
*Exercise 5 Answer:*
The box plot shows that a person's self-reported general health appears to be positively correlated to their body mass index.

I decided to pick exerany as the categorical variable to examine, I think exercise may also correlate with bmi.

```{r ex-five, eval=TRUE}
boxplot(bmi ~ cdc$exerany)
```

Looking at the boxplots, it does NOT appear that whether someone has exercised in the past month correlates to their BMI.
...
*Number 3 Answer:*  
wdiff is an integer.  If wdiff is zero, it means the individual's desired weight and actual weight are the same.  If wdiff is positive, it means the individual would like their weight to be more than it is, and if it is negative, it means the individual would like their weight to be less than it is.
...
```
It's a unimodal distribution skewed to the left.  The mean is below zero, which means on average most people would like their weight to be less than it is.  Because of the skew you can see that many individuals would like to weight way less than they do, while it is less so in the other direction.  

*Number 5 Answer:*  
```{r wdiff-m-vs-f, EVAL = TRUE}
summary(subset(wdiff, cdc$gender == "m"))
summary(subset(wdiff, cdc$gender == "f"))
boxplot(wdiff ~ cdc$gender)
```
Looking at both the summary stats and the side-by-side box plots, it appears many more women see themselves as needing to lose weight, and to greater extents then men on average, though both have a similar distribution.  It is just more amplified for women and skewed more to the negative side than men.  

*Number 6 Answer:*  
```{r within-one-sd, EVAL = TRUE}
nrow(subset(cdc, weight >= mean(cdc$weight) - sd(cdc$weight) & weight <= mean(cdc$weight) + sd(cdc$weight))) / nrow(cdc)
```  
